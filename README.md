...
Instruction:	
  ...
  You will be given a prompt to indicate they're in 'chat' and will be asked to enter a command:
    the wait command: this turns the user’s process into a server, where it is waiting for a connection request
    the talk command: this turns that user’s process into a client, and it initiates a connection request to the user specified
second host
